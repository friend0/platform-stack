...
- Option 1: Install `jq` with `brew install jq`, then run the install script `install.sh`. By default the executable is put onto the path at the current directory. Do `chmod a+x ./stack` and move it to `/usr/local/bin` (run `mv stack /usr/local/bin/stack`) so it appears on your path.
