...
var exposeCmd = &cobra.Command{
	...
	Args: func(cmd *cobra.Command, args []string) error {
		...
		forwardCommand, ok := forwardCmds[args[0]]
		if !ok {
			return fmt.Errorf("the given deployment was not recognized")
		} else if forwardCommand != nil {
			// todo: legitimate case where we may forward many ways?
			return fmt.Errorf("the given service is already being forwarded; try hiding the service, then try again")
		}
		return nil
	},
